...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      def all_aux (fx: List[Future[T]], temp: List[T]): Future[List[T]] = {
        fx match {
          case Nil => always (temp)
          case hd::tl => {
            Try (hd) match {
              case Success (value: T) => all_aux (tl, temp.union (List(value)))
              case Failure (error) => {
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.map (x => x.onComplete (p.complete (_)))
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = {
      if (f.isCompleted) Try(f).asInstanceOf[T]
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S] ()
      f.onComplete (x => p.success (cont (f)))
